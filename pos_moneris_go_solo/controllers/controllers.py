...
class PosMonerisTerminalAPI(http.Controller):
    @http.route('/api/auth', type='json', auth="public", cors="*")
    def authenticate(self, **post):
        ...
        http.request.session.authenticate(db_name, credential)
        session_info = request.env['ir.http'].session_info()
        today_date = datetime.today()
        # api_key = request.env['res.users.apikeys']._generate('api', 'Auth key',today_date )

        expiration_time = datetime.utcnow() + timedelta(hours=15)
        api_key = request.env['res.users.apikeys']._generate('api', 'Auth key', expiration_time)
    ...
    def get_draft_go_payment(self, **params):
        _logger.info('get_draft_go_payment.')
        ...
        print('moneris_payments', moneris_payments)

        # Generate API response
        data = [payment.get_draft_go_payment_data() for payment in moneris_payments]
    ...
    @http.route('/monerisgo/update_order_info', type='json', auth="public", cors="*")
    def moneris_go_get_order_info(self, **post):
        print('monerisgo',post)
        payment_id = post['params']['data']['payment_id']
        order_id = post['params']['data']['order_id']
        if payment_id:
            pay_obj = request.env['pos.payment'].sudo().browse(payment_id)
            for p in pay_obj:
                p.sudo().write({
                    "payment_status": 'cancel'
                })
        if order_id:
            pay_obj = request.env['pos.payment'].sudo().search([('pos_order_id', '=', order_id)])
            for p in pay_obj:
                if p.payment_status == 'cancel':
                    p.sudo().write({
