...
export class RefundMonerisPaymentPopupWidget extends DebugWidget {
    ...
    async confirm() {
        ...
        if (selected_moneris_payment.amount <= Math.abs(paymentline.get_amount())) {
            ...
            body = _t('Are you sure you want to refund ') + ': ' + this.env.pos.format_currency(refund_amount) + ' ?';
        } else {
            refund_amount = Math.abs(paymentline.get_amount());
            body = _t('Are you sure you want to refund partial amount ') + ': ' + this.env.pos.format_currency(refund_amount) + ' ?';
        }
